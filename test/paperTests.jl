...
#Example: errortest(9,[0,1,2,10],create_plot=1,dir="plots/")
  ...
  if create_plot == 1
    ...
    p=plot(xgroup=op, x=methods, y=errors, color=colors,
           ...
           Scale.y_log10, Guide.xlabel(nothing),Guide.Title("R=$R"),
           Guide.ylabel("Error"),
 	         Scale.x_discrete,Scale.color_discrete_manual(c.f(4)[3],c.f(4)[4]));
...
#Example: hosvdrandtest(50,collect(2:2:20),3,create_plot=1, dir="plots")
  ...
  L=length(ranks);
  println("\nCreating random tensors of order $N, size $S and ranks $ranks.");
  ...
  for R in ranks
    ...
    println("-> Tucker tensors of order $N, size $S and rank $R.");
    i+=1;
  end
