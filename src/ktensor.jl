...
"""
mutable struct ktensor{T<:Number}
	lambda::Vector{T}
	fmat::MatrixCell
	isorth::Bool
...
ktensor(lambda::Vector{T},fmat::MatrixCell,isorth::Bool) where T=ktensor{T}(lambda,fmat,isorth)
ktensor(lambda::Vector{T},fmat::MatrixCell) where T=ktensor{T}(lambda,fmat,true)
...
ktensor(lambda::Vector{T},fmat::Array{Matrix{T1}},isorth::Bool) where {T,T1<:Number}=ktensor{T}(lambda,MatrixCell(fmat),isorth)
ktensor(lambda::Vector{T},fmat::Array{Matrix{T1}}) where {T,T1<:Number}=ktensor{T}(lambda,MatrixCell(fmat),true)
ktensor(fmat::Vector{Matrix{T}}) where {T<:Number}=ktensor(MatrixCell(fmat))
"""
...
"""
function arrange(X::ktensor{T},mode=-1) where {T<:Number}
    N=ndims(X)
...
function arrange(X::ktensor{T},perm::Vector{D}) where {T<:Number,D<:Integer}
    N=ndims(X)
...
function display(X::ktensor{T},name="ktensor") where {T<:Number}
...
extract(X::ktensor{T},factors::Integer) where {T<:Number}=extract(X,[factors])
...
"""
function fixsigns(X::ktensor{T}) where {T<:Number}
    N=ndims(X)
    sgn=zeros(Int,N)
...
function full(X::ktensor{T}) where {T<:Number}
  reshape(Array(X.lambda'*khatrirao(X.fmat[end:-1:1])'),size(X)...)
end

#Inner product of two ktensors. **Documentation in tensor.jl.
function innerprod(X1::ktensor{T1},X2::ktensor{T2}) where {T1<:Number,T2<:Number}
...
function innerprod(X1::ktensor{T},X2) where {T<:Number}
...
innerprod(X1,X2::ktensor{T}) where {T<:Number}=innerprod(X2,X1)

#Two ktensors are equal if they have equal components. **Documentation in ttensor.jl.
function isequal(X1::ktensor{T1},X2::ktensor{T2}) where {T1<:Number,T2<:Number}
...
-(X1::ktensor{T1},X2::ktensor{T2}) where {T1<:Number,T2<:Number}= minus(X1,X2)

#Scalar times tensor. **Documentation in ttensor.jl.
function mtimes(α::Number,X::ktensor{T}) where {T<:Number}
	ktensor(α*X.lambda,X.fmat);
end
*(α::T1,X::ktensor{T2}) where {T1<:Number,T2<:Number}=mtimes(α,X)
*(X::ktensor{T1},α::T2) where {T1<:Number,T2<:Number}=*(α,X)

#Matricized ktensor times Khatri-Rao product. **Documentation in tensor.jl.
function mttkrp(X::ktensor{T},M::MatrixCell,n::Integer) where {T<:Number}
  N=ndims(X)
...
mttkrp(X::ktensor{T1},M::Array{Matrix{T2}},n::Integer) where {T1<:Number,T2<:Number}=mttkrp(X,MatrixCell(M),n)
...
"""
function ncomponents(X::ktensor{T}) where {T<:Number}
  length(X.lambda)
end

#Number of modes of a ttensor. **Documentation in Base.
function ndims(X::ktensor{T}) where {T<:Number}
  length(X.fmat)
end
...
"""
function normalize(X::ktensor{T},mode=-1;normtype=2,factor=-1) where {T<:Number}
  if mode=="sort"
    mode=-2
  end
  N=ndims(X)
...
function nvecs(X::ktensor{T},n::Integer,r=0;flipsign=false) where {T<:Number}
    M = X.lambda * X.lambda'
...
function permutedims(X::ktensor{T},perm::Vector{D}) where {T<:Number,D<:Integer}
    N=ndims(X)
...
function plus(X1::ktensor{T1},X2::ktensor{T2}) where {T1<:Number,T2<:Number}
	@assert(size(X1) == size(X2),"Dimension mismatch.")
  lambda=[X1.lambda; X2.lambda]
	fmat=Matrix[[X1.fmat[n] X2.fmat[n]] for n=1:ndims(X1)] #concatenate factor matrices
	ktensor(lambda,fmat)
end
+(X1::ktensor{T1},X2::ktensor{T2}) where {T1<:Number,T2<:Number}=plus(X1,X2)
...
"""
function redistribute(X::ktensor{T},mode::Integer) where {T<:Number}
    lambda=deepcopy(float(X.lambda))
    fmat=deepcopy(X.fmat)
    for r = 1:ncomponents(X)
...
function size(X::ktensor{T}) where {T<:Number}
  tuple([size(X.fmat[n],1) for n=1:length(X.fmat)]...)
end
#Size of n-th mode of a ktensor.
function size(X::ktensor{T},n::Integer) where {T<:Number}
  size(X.fmat[n],1)
end

function Base.show(io::IO,X::ktensor{T}) where {T<:Number}
    display(X)
end

#Mode-n matricization of a ktensor. **Documentation in tensor.jl.
tenmat(X::ktensor{T},n::Integer) where {T<:Number}=tenmat(full(X),n)
...
"""
function tocell(X::ktensor{T},mode::Integer) where {T<:Number}
    Y = normalize(X,mode)
    Y.fmat
end
function tocell(X::ktensor{T}) where {T<:Number}
...
"""
function ttensor(X::ktensor{T}) where {T<:Number}
...
ttm(X::ktensor{T},M::MatrixCell,modes::AbstractRange{D},t='n') where {T<:Number,D<:Integer}=ttm(X,M,collect(modes),t)
ttm(X::ktensor{T1},M::Matrix{T2},n::Integer,t='n') where {T1<:Number,T2<:Number}=ttm(X,[M],[n],t)
ttm(X::ktensor{T},M::MatrixCell,t::Char) where {T<:Number}=ttm(X,M,1:length(M),t)
ttm(X::ktensor{T},M::MatrixCell) where {T<:Number}=ttm(X,M,1:length(M))
function ttm(X::ktensor{T},M::MatrixCell,n::Integer,t='n') where {T<:Number}
	if n>0
		ttm(X,M[n],n,t)
	else
...
#If array of matrices isn't defined as MatrixCell, but as M=[M1,M2,...,Mn]:
ttm(X::ktensor{T1},M::Array{Matrix{T2}},modes::Vector{D},t='n') where {T1<:Number,T2<:Number,D<:Integer}=ttm(X,MatrixCell(M),modes,t)
ttm(X::ktensor{T1},M::Array{Matrix{T2}},modes::AbstractRange{D},t='n') where {T1<:Number,T2<:Number,D<:Integer}=ttm(X,MatrixCell(M),modes,t)
ttm(X::ktensor{T1},M::Array{Matrix{T2}},t::Char) where {T1<:Number,T2<:Number}=ttm(X,MatrixCell(M),t)
ttm(X::ktensor{T1},M::Array{Matrix{T2}}) where {T1<:Number,T2<:Number}=ttm(X,MatrixCell(M))
ttm(X::ktensor{T1},M::Array{Matrix{T2}},n::Integer,t='n') where {T1<:Number,T2<:Number}=ttm(X,MatrixCell(M),n,t)

#Mode-n multiplication ktensor and vector. **Documentation in tensor.jl.
#t='t' transposes matrices
function ttv(X::ktensor{T},V::VectorCell,modes::Vector{D}) where {T<:Number,D<:Integer}
  N=ndims(X)
...
ttv(X::ktensor{T1},v::Vector{T2},n::Integer) where {T1<:Number,T2<:Number}=ttv(X,Vector[v],[n])
ttv(X::ktensor{T},V::VectorCell,modes::AbstractRange{D}) where {T<:Number,D<:Integer}=ttv(X,V,collect(modes))
ttv(X::ktensor{T},V::VectorCell) where {T<:Number}=ttv(X,V,1:length(V))
function ttv(X::ktensor{T},V::VectorCell,n::Integer) where {T<:Number}
	if n>0
		ttm(X,V[n],n)
	else
		modes=setdiff(1:ndims(X),-n)
...
ttv(X::ktensor{T1},V::Array{Vector{T2}},modes::Vector{D}) where {T1<:Number,T2<:Number,D<:Integer}=ttv(X,VectorCell(V),modes)
ttv(X::ktensor{T1},V::Array{Vector{T2}},modes::AbstractRange{D}) where {T1<:Number,T2<:Number,D<:Integer}=ttv(X,VectorCell(V),modes)
ttv(X::ktensor{T1},V::Array{Vector{T2}}) where {T1<:Number,T2<:Number}=ttv(X,VectorCell(V))
ttv(X::ktensor{T1},V::Array{Vector{T2}},n::Integer) where {T1<:Number,T2<:Number}=ttv(X,VectorCell(V),n)

#**Documentation in ttensor.jl.
uminus(X::ktensor{T}) where {T<:Number}=mtimes(-1,X)
